...
current_case = st.sidebar.selectbox(
    ...
    ["quotes", "ecommerce"],
    index=0 if config.CURRENT_CASE == "quotes" else 1
)
...
# Core data pipeline verification
df_loaded = False
raw_df = pd.DataFrame()

if config.OUTPUT_XLSX.exists():
    try:
        raw_df = pd.read_excel(config.OUTPUT_XLSX)
        df_loaded = True
    except Exception as e:
        st.sidebar.warning("Excel read skipped, trying CSV alternative...")

if not df_loaded and config.OUTPUT_CSV.exists():
    try:
        raw_df = pd.read_csv(config.OUTPUT_CSV)
        df_loaded = True
    except Exception as e:
        st.error(f"Error loading CSV data file: {e}")

if raw_df.empty:
    st.warning("⚠️ No valid extracted data found. Please run `main.py` first to generate datasets.")
else:
    ...
    st.sidebar.info(
        f"**Active Mode:** {config.CURRENT_CASE.upper()}\n\n"
    ...
    with tab_data:
        ...
        with col_down1:
            st.download_button(
                ...
                file_name=f"cleaned_{config.CURRENT_CASE}.xlsx",
        ...
        with col_down2:
            st.download_button(
                ...
                data=df.to_csv(index=False).encode('utf-8'),
                file_name=f"cleaned_{config.CURRENT_CASE}.csv",
                mime="text/csv"
            )

        # Tab 2: Statistical Analytics Visualizations
        with tab_analytics:
            st.subheader(f"Exploratory Data Analysis — `{config.CURRENT_CASE.upper()}` Mode")

            # Общие настройки осей для кофейной палитры
            axis_theme = dict(
                title_font=dict(color="#5C4033", size=14),
                tickfont=dict(color="#4A3B32", size=12),
                gridcolor="#E6D5C3",  # Делаем линии сетки мягкими кофейными
                ...
                with g1:
                    ...
                    if flat_tags:
                        ...
                        fig_tags.update_layout(
                            ...
                            # Делаем шрифт и деления цветовой шкалы (colorbar) кофейными
                ...
                with g2:
                    ...
                    if not author_counts.empty:
                        fig_authors = px.pie(author_counts, values="Quotes Count", names="Author",
                                             color_discrete_sequence=["#5C4033", "#6F4E37", "#8B5A2B", "#D7C4B7",
                                                                      "#E6D5C3"],
                                             hole=0.3)
                        fig_authors.update_layout(
                            ...
                            legend=dict(
                                orientation="h",
                                y=-0.1,
                                font=dict(color="#4A3B32")  # Цвет текста легенды
                            )
                        )
